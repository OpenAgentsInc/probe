...
if ! gcloud iam service-accounts describe "$PROBE_FORGE_SERVICE_ACCOUNT_EMAIL" \
  ...
  gcloud iam service-accounts create "$PROBE_FORGE_SERVICE_ACCOUNT_NAME" \
    --project "$GCP_PROJECT" \
    --display-name "Probe hosted Forge dogfood worker" >/dev/null
fi
...
if firewall_rule_exists "oa-allow-probe-hosted-iap-ssh"; then
  log "Updating firewall rule: oa-allow-probe-hosted-iap-ssh"
  gcloud compute firewall-rules update "oa-allow-probe-hosted-iap-ssh" \
    --project "$GCP_PROJECT" \
    --allow tcp:22 \
    --target-tags "$PROBE_FORGE_TAG" \
    --source-ranges "35.235.240.0/20" >/dev/null
else
  log "Creating firewall rule: oa-allow-probe-hosted-iap-ssh"
  gcloud compute firewall-rules create "oa-allow-probe-hosted-iap-ssh" \
    --project "$GCP_PROJECT" \
    --network "$OA_VPC" \
    --allow tcp:22 \
    --target-tags "$PROBE_FORGE_TAG" \
